...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    pub(super) fn left(&mut self, _: &MoveLeft, _: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.is_single_line() && self.selected_range.is_empty() && self.cursor() == 0 {
            cx.propagate();
            return;
        }
        self.pause_blink_cursor(cx);
    ...
    pub(super) fn up(&mut self, action: &MoveUp, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.is_single_line() {
            // Up means nothing in a single-line input, so let the keystroke
            // reach whatever is around it (a list, a dialog). A fork patch:
            // upstream swallows it here.
            cx.propagate();
            return;
        }
    ...
    pub(super) fn down(&mut self, action: &MoveDown, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.is_single_line() {
            // Down means nothing in a single-line input, so let the keystroke
            // reach whatever is around it (a list, a dialog). A fork patch:
            // upstream swallows it here.
            cx.propagate();
            return;
