...
impl Popup {
    ...
    /// Shift the popup — occlude area included — by this offset relative to
    /// the resolved anchor point, default `(0, 0)`.
    ///
    /// A fork addition (3b9f0da6), carried here rather than in the styled
    /// `HoverCard` because this is where the anchor point is resolved. Elane
    /// uses it to keep a file-table hover card clear of the cells it would
    /// otherwise cover.
